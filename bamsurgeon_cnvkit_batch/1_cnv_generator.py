...
# Just follow the comments to set everything up. The resulting txt files will be placed into "sim" subdirectory of the script location.
...
# Load Files
if BLACKLIST_FLAG == 1:
    bl_dir = os.path.join(BASE_DIR, "blacklist")
    if not os.path.exists(bl_dir): os.makedirs(bl_dir)
    bl_path = os.path.join(bl_dir, blacklist_name)
    download_file(blacklist_url, bl_path)
    # [FIX] Explicitly tell it this is a BED file
    load_intervals(bl_path, blacklist_dict, file_format='bed')

if AVOID_REPEATS == 1:
    sr_dir = os.path.join(BASE_DIR, "simple_repeat_" + REF_VERSION)
    if not os.path.exists(sr_dir): os.makedirs(sr_dir)
    sr_path = os.path.join(sr_dir, "simpleRepeat.txt.gz")
    download_file(simplerepeat_url, sr_path)
    # [FIX] Explicitly tell it this is a UCSC db file
